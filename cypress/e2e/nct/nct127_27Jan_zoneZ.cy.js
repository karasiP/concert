describe('template spec', () => {
  it('passes', () => {
    ...
    //Select date 27 JAN
    cy.get('*[name^="custCardType"]').eq(0).click({force: true});
    //Click seat available and select zone (Z) 
    cy.get('area').eq(1).click({ force: true })
    // cy.get('area[class="p_Z"]').click({ force: true })
    //Select seat
